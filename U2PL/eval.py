...
def get_parser():
    ...
    parser.add_argument("--config", type=str, default="./experiments/customs/ours/config.yaml")
    parser.add_argument(
        "--model_path",
        type=str,
...
def valiadte_whole(
    ...
    file=open("unlabeled.txt","r").read().splitlines()
    for i, input_pth in enumerate(file):
        data_time.update(time.time() - end)
        print(input_pth)
        input_pth='/home/congvu/Semi_Segment/U2PL/data/customs/'+input_pth
        # input_pth=input_pth.replace("train_images","train")
        # input_pth=input_pth[:len(input_pth)-4]
        image = Image.open(input_pth).convert("RGB")
        image = np.asarray(image).astype(np.float32)
        image = (image - mean) / std
        image = torch.Tensor(image).permute(2, 0, 1)
